 ...
 * MusicalChairsOverlay uses for its hovering avatars), while the wrapper's (x, y) still orbits
      ...
      <motion.div style={{ rotate: counterRotation }} className="relative flex flex-col items-center">
        {selected && spotlightActive && (
          <motion.div
            ...
            className="absolute -inset-2.5 rounded-full bg-fuchsia-400/50 blur-lg"
          />
        )}

        {/* Pennant name tag — flat top, tapers to a point aimed down at the avatar. */}
        <div
          className="relative -mb-1 whitespace-nowrap px-2 pb-2 pt-0.5 text-[9px] font-bold text-white ring-1 ring-inset ring-white/35 backdrop-blur-md"
          style={{
            clipPath: "polygon(6% 0%, 94% 0%, 100% 58%, 50% 100%, 0% 58%)",
            background: `linear-gradient(180deg, ${tint}dd, ${tint}55 70%, transparent)`,
          }}
        >
          <bdi className="block max-w-[62px] truncate drop-shadow-[0_1px_2px_rgba(5,3,13,0.8)]">{player.displayName}</bdi>
        </div>

        <div className="relative">
          <PlayerAvatar displayName={player.displayName} avatarUrl={player.avatarUrl} size={44} />
          <span className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-br from-primary to-fuchsia-600 text-[10px] font-black text-white ring-2 ring-canvas">
            {player.number}
          </span>
        </div>
      </motion.div>
  ...
  return (
    <div className={`relative ${sizeClass}`}>
        ...
        <svg viewBox="-120 -120 240 240" className="h-full w-full">
          <defs>
            {WHEEL_SEGMENTS.map(([light, dark], k) => (
              <radialGradient key={k} id={`ovsw-seg-${k}`} cx="0" cy="0" r="100" gradientUnits="userSpaceOnUse">
              ...
              <g key={p.handle}>
                {players.length > 1 ? (
                  <path
                    ...
                    fill={`url(#ovsw-seg-${gi})`}
                    stroke="rgba(255,255,255,0.28)"
                    strokeWidth="1"
                  />
                ) : (
                  <circle r="100" fill="url(#ovsw-seg-0)" />
                )}
      ...
      <svg viewBox="-120 -120 240 240" className="pointer-events-none absolute inset-0 h-full w-full">
        <defs>
          <radialGradient id="ovsw-vig" cx="0" cy="0" r="100" gradientUnits="userSpaceOnUse">
            <stop offset="70%" stopColor="rgba(5,3,13,0)" />
            <stop offset="100%" stopColor="rgba(5,3,13,0.5)" />
          </radialGradient>
          <linearGradient id="ovsw-rim" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#c084fc" />
            <stop offset="50%" stopColor="#7c3aed" />
            <stop offset="100%" stopColor="#d946ef" />
          </linearGradient>
          <radialGradient id="ovsw-hub" cx="0" cy="0" r="34" gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor="#2a1245" />
            <stop offset="100%" stopColor="#0a0616" />
          </radialGradient>
        </defs>
        <circle r="100" fill="url(#ovsw-vig)" />
        <circle r="107" fill="none" stroke="url(#ovsw-rim)" strokeWidth="9" />
        ...
        <circle r="30" fill="url(#ovsw-hub)" stroke="url(#ovsw-rim)" strokeWidth="2.5" />
      ...
      >
        <svg
          ...
          viewBox="0 0 36 48"
          className="drop-shadow-[0_0_10px_rgba(192,132,252,0.9)]"
        >
          <defs>
            <linearGradient id="ovsw-ptr" x1="0" y1="0" x2="0" y2="1">
...
function ShieldExplosion({ burstKey }: { burstKey: string }) {
    ...
    <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center overflow-hidden">
      ...
      {[0, 0.12, 0.24].map((delay, i) => (
        <motion.div
          ...
          className="absolute h-48 w-48 rounded-full border-4 border-amber-300/80"
        />
      ))}
      {particles.map((pt, i) => (
        <motion.span
...
function SpinResultReveal({ state }: { state: SpinWheelState }) {
  ...
  if (action.type === "SHIELD_BLOCKED") {
    return (
      <motion.div
        animate={{ x: [0, -12, 10, -8, 5, -2, 0], y: [0, 8, -9, 6, -3, 2, 0] }}
        transition={{ duration: 0.65, ease: "easeOut" }}
        className="relative flex flex-col items-center gap-3 rounded-3xl border border-amber-400/50 bg-gradient-to-br from-amber-500/25 to-orange-600/15 p-8 text-center shadow-glass backdrop-blur-2xl"
      >
        ...
        <PlayerAvatar displayName={action.target.displayName} avatarUrl={action.target.avatarUrl} size={84} />
  ...
  if (action.type === "WITHDREW") {
    return (
      <div className="flex flex-col items-center gap-3 rounded-3xl border border-sky-400/40 bg-gradient-to-br from-sky-500/20 to-indigo-600/15 p-8 text-center shadow-glass backdrop-blur-2xl">
        <motion.div initial={{ y: 0 }} animate={{ y: [-5, 5, -5] }} transition={{ duration: 2, repeat: Infinity }} className="text-4xl">
          🕊️
        </motion.div>
        <PlayerAvatar displayName={action.picker.displayName} avatarUrl={action.picker.avatarUrl} size={84} />
        <h3 className="text-2xl font-extrabold text-sky-300">
          <bdi>{action.picker.displayName}</bdi> انسحب بشرف!
        </h3>
        <p className="text-sm text-ink-muted">كتب 0 وسلّم نفسه بدل ما يطرد حد 🫡</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative flex flex-col items-center gap-3 rounded-3xl border border-red-400/40 bg-gradient-to-br from-red-500/20 to-rose-600/15 p-8 text-center shadow-glass backdrop-blur-2xl">
      <motion.div initial={{ scale: 0, rotate: -30 }} animate={{ scale: [0, 1.5, 1] }} transition={{ duration: 0.6 }} className="text-4xl">
        💥
      </motion.div>
      <motion.div
      ...
      <p className="text-sm text-ink-muted">{viaText}</p>
    </div>
  );
}
...
function PlayerChip({ name, avatarUrl }: { name: string; avatarUrl?: string | null }) {
  return (
    <motion.span
      ...
      className="flex items-center gap-1.5 rounded-full border border-accent/40 bg-canvas-soft/80 py-1.5 pl-4 pr-1.5 text-sm font-medium text-ink shadow-glass backdrop-blur-xl"
    >
      <bdi>{name}</bdi>
  ...
  return (
    <div className="flex max-h-[440px] flex-col gap-1.5 overflow-y-auto rounded-3xl border border-glass-border bg-canvas-soft/70 p-4 shadow-glass backdrop-blur-2xl">
      ...
      {state.shieldsTotal > 0 && (
        <div className="mb-1 flex items-center gap-1.5 rounded-xl bg-amber-400/10 px-2 py-1.5 text-[11px] font-semibold text-amber-300">
      ...
      {state.players.map((p) => (
        ...
        >
          <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white/10 text-[10px] font-bold text-accent">
            {p.number}
          </span>
      ...
      {state.players.length === 0 && <p className="text-xs text-ink-muted">لسه محدش دخل</p>}
    </div>
  );
}
          ...
          >
            ...
            {state.phase === "PICKING" && selected && (
              <motion.div
                ...
                className="flex flex-col items-center gap-1.5 rounded-3xl border border-accent/40 bg-canvas-soft/80 px-6 py-3 text-center shadow-glass backdrop-blur-2xl"
              >
                <div className="flex items-center gap-2.5">
                  <PlayerAvatar displayName={selected.displayName} avatarUrl={selected.avatarUrl} size={38} />
                  <p className="text-sm font-bold">
                    يا <bdi className="text-accent">{selected.displayName}</bdi> — اكتب رقم اللاعب اللي هتطرده! 😈
                  </p>
                  {secondsLeft !== null && <span className="text-2xl font-extrabold text-accent">{secondsLeft}</span>}
                </div>
...
export function SpinWheelOverlay({ state, chat }: { state: SpinWheelState | null; chat: ChatItem[] }) {
  if (!state) {
    return (
        ...
        <AnimatePresence mode="wait">
          {state.phase === "WAITING_FOR_PLAYERS" && (
            ...
            >
              ...
              <h1 className="text-2xl font-bold">عجلة الحظ</h1>
              <p className="mt-2 text-ink-muted">
                اكتب <bdi className="font-semibold text-accent">{state.settings.joinCommand}</bdi> عشان تدخل اللعبة!
              </p>
              <p className="mt-2 text-xs text-ink-muted">
                العجلة بتختار لاعب يطرد حد برقمه — <span className="font-semibold text-ink">0</span> ينسحب،{" "}
                <span className="font-semibold text-ink">00</span> طرد عشوائي، وفي دروع مخفية 🛡️
              </p>
