    ...
    <div className="flex max-h-[440px] flex-col gap-1.5 overflow-y-auto rounded-3xl border border-glass-border bg-canvas-soft/70 p-4 shadow-glass backdrop-blur-2xl">
      ...
      {ranked.map((p, i) => (
        <div key={p.handle} className={`flex items-center gap-2 rounded-xl px-2 py-1.5 ${i < 3 ? "bg-white/5" : ""}`}>
          ...
          <PlayerAvatar displayName={p.displayName} avatarUrl={p.avatarUrl} size={26} ring={false} />
          <bdi className="truncate text-xs">{p.displayName}</bdi>
          <span className="mr-auto shrink-0 text-xs font-bold text-amber-400">{p.score}</span>
        </div>
      ))}
...
function LogoTile({ slug, size }: { slug: string; size: number }) {
  return (
    <div
      ...
      className="relative overflow-hidden rounded-3xl bg-white p-[14%] shadow-[0_14px_30px_rgba(251,191,36,0.25)] ring-4 ring-amber-300/60"
    >
...
function LogoArena({ state }: { state: LogosState }) {
  ...
  return (
    <div className="relative flex h-[440px] w-[440px] flex-col items-center justify-center gap-4 overflow-hidden rounded-3xl border border-glass-border bg-gradient-to-br from-amber-500/10 via-canvas-soft to-canvas-soft p-8 shadow-glass">
      <motion.div
        animate={{ opacity: [0.35, 0.65, 0.35], scale: [1, 1.08, 1] }}
        transition={{ duration: 2.6, repeat: Infinity, ease: "easeInOut" }}
        className="pointer-events-none absolute inset-8 rounded-full bg-amber-400/20 blur-3xl"
      />
      ...
      <AnimatePresence mode="wait">
        {!revealed ? (
          ...
          >
            {state.logoSlug && <LogoTile slug={state.logoSlug} size={190} />}
            <p className="flex items-center gap-1.5 text-sm font-bold text-white">
              <Storefront size={16} weight="fill" className="text-amber-400" />
              الماركة دي إيه؟
            </p>
          </motion.div>
        ) : (
          <motion.div
            ...
            className="flex flex-col items-center gap-3"
          >
            {state.logoSlug && <LogoTile slug={state.logoSlug} size={120} />}
            <motion.div
              initial={{ scale: 0, rotate: -20 }}
              animate={{ scale: 1, rotate: -8 }}
              transition={{ type: "spring", stiffness: 300, damping: 12, delay: 0.1 }}
              className="flex items-center gap-1.5 rounded-2xl border-2 border-amber-400 px-3 py-1"
            >
              <SealCheck size={18} weight="fill" className="text-amber-400" />
              <p className="text-xl font-extrabold text-amber-300">{state.brandName}</p>
            </motion.div>
            {state.lastWinner ? (
              <motion.div
...
export function LogosOverlay({ state, chat }: { state: LogosState | null; chat: ChatItem[] }) {
  if (!state) {
    return (
      ...
      <div className="flex flex-1 items-center justify-center">
        <AnimatePresence mode="wait">
          {state.phase === "WAITING_TO_START" && (
            ...
            >
              <Storefront size={48} weight="fill" className="mx-auto mb-2 text-amber-400" />
              <h1 className="text-2xl font-bold">شعارات</h1>
              <p className="mt-2 text-ink-muted">
                {state.settings.totalRounds} شعار ماركة هيظهروا واحد ورا التاني — جاوب باسم الماركة في الشات!
              </p>
            </motion.div>
          )}
              ...
              <div className="flex flex-col items-center gap-4 text-center">
                {state.winner ? (
                  ...
                  <div className="rounded-3xl border border-glass-border bg-canvas-soft/70 p-8 shadow-glass backdrop-blur-2xl">
                    <Storefront size={40} className="mx-auto mb-3 text-ink-muted" />
                    <p className="text-ink-muted">خلصت الشعارات من غير ما حد يسجل نقطة</p>
                  </div>
                )}
              </div>
              <LeaderboardPanel players={state.players} />
