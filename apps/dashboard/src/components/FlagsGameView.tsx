...
function FlagImage({ code, countryName, width, height }: { code: string; countryName: string | null; width: number; height: number }) {
  return (
    <div
      style={{ width, height }}
      className="relative overflow-hidden rounded-2xl ring-2 ring-white/40 shadow-[0_18px_36px_rgba(5,3,13,0.55)]"
    >
...
function FlagArena({ state }: { state: FlagsState }) {
  ...
  return (
    <div className="relative flex h-[min(72vh,640px)] w-[min(72vh,640px)] flex-col items-center justify-center gap-6 overflow-hidden rounded-3xl border border-glass-border bg-gradient-to-br from-primary/15 via-canvas-elevated to-canvas-elevated p-10">
      <motion.div
      ...
      <AnimatePresence mode="wait">
        {!revealed ? (
          ...
          >
            ...
            >
              {state.countryCode && (
                <FlagImage code={state.countryCode} countryName={state.countryName} width={300} height={225} />
              )}
            </motion.div>
            <p className="text-xl font-bold text-white">العلم ده لمين؟ 🤔</p>
          </motion.div>
        ) : (
          <motion.div
            key="reveal"
          ...
          >
            ...
            <p className="text-3xl font-extrabold text-accent">{state.countryName}</p>
            {state.lastWinner ? (
              <motion.div
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center">
        <AnimatePresence mode="wait">
          {state.phase === "WAITING_TO_START" && (
            ...
            >
              <FlagBanner size={64} weight="fill" className="mx-auto mb-2 text-accent" />
              <h3 className="text-3xl font-bold">أعلام</h3>
              <p className="mt-3 text-base text-ink-muted">
                {state.settings.totalRounds} علم هيظهروا واحد ورا التاني — هيبدأ أول علم دلوقتي، جاوب باسم الدولة في الشات!
              </p>
            </motion.div>
          )}
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center gap-6 text-center">
                {state.winner ? (
                  ...
                  <NoWinnerScreen message="خلصت الأعلام من غير ما حد يسجل نقطة 😅" onNewRound={onNewRound} />
                )}
              </div>
              <div className="flex min-h-0 flex-col gap-4">
                <LeaderboardPanel players={state.players} />
