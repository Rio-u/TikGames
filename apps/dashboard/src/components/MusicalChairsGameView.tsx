...
function PlayerChip({ name, avatarUrl, eliminated }: { name: string; avatarUrl?: string | null; eliminated?: boolean }) {
  return (
    <motion.span
      ...
      className={`flex items-center gap-1.5 rounded-full border py-1.5 pl-4 pr-1.5 text-sm font-medium ${
        eliminated
          ? "border-glass-border bg-canvas-elevated/60 text-ink-muted line-through"
          : "border-accent/40 bg-canvas-elevated/80 text-ink"
      }`}
    >
      {name}
...
/**
 * Snapshots the ranking once per completed round instead of re-sorting live on every elimination
 * tick — the board should read "here's how the previous round ended," not flicker mid-reveal.
 */
function useRoundSnapshot(state: MusicalChairsState) {
  const [snapshot, setSnapshot] = useState({ round: state.round, players: state.players });
  const lastRoundRef = useRef(state.round);

  useEffect(() => {
    if (state.phase === "FINISHED" || state.round !== lastRoundRef.current) {
      setSnapshot({ round: state.round, players: state.players });
      lastRoundRef.current = state.round;
    }
  }, [state.round, state.phase, state.players]);

  return snapshot;
}

function LeaderboardPanel({ round, players }: { round: number; players: MusicalChairsPlayer[] }) {
  ...
  return (
    <div className="flex min-h-0 flex-[1.2] flex-col gap-1.5 overflow-y-auto rounded-2xl border border-glass-border bg-canvas-elevated/50 p-4">
      <div className="mb-1 flex items-center gap-1.5 text-sm font-semibold">
        ...
        {round > 0 ? `ترتيب الجولة السابقة (${round})` : "الترتيب"}
      </div>
      {ranked.map((p, i) => (
...
/**
 * One persistent element for the whole game (mounted from setup through the last round), with
 * play/pause driven by `active` — never re-created per round, because strict-autoplay browsers
 * (Brave blocks autoplay per-site by default; Chrome wants a prior user gesture) grant playback
 * to an element once it has played inside a real gesture. Until then, any click on the page
 * silently unlocks it at volume 0 — the streamer's own "ابدأ الجولات" click counts, since
 * document-level pointerdown handlers still run in the gesture context. If a music phase still
 * finds it blocked, a persistent enable button appears instead of failing silently.
 */
    ...
    <>
      ...
      {blocked && (
        <button
          ...
          className="absolute left-4 top-4 z-20 flex items-center gap-1.5 rounded-full border border-accent/40 bg-accent/15 px-3 py-1.5 text-xs font-medium text-accent backdrop-blur-md"
        >
  ...
  return (
    <div className="relative flex h-[min(72vh,640px)] w-[min(72vh,640px)] items-center justify-center">
        ...
        {seated.map((p) => {
          ...
          return (
            <motion.div
              ...
              className="absolute left-1/2 top-1/2 -ml-9 -mt-11 flex flex-col items-center gap-1.5"
            >
              <PlayerAvatar displayName={p.displayName} avatarUrl={p.avatarUrl} size={68} />
              <span className="max-w-[110px] truncate rounded-full bg-accent/90 px-2.5 py-1 text-xs text-canvas-elevated">
                {p.displayName}
              </span>
            </motion.div>
          );
          ...
          {hovering.map((p, i) => {
            ...
            return (
              <motion.div
                ...
                className="absolute left-1/2 top-1/2 -ml-9 -mt-11"
              >
                <motion.div style={{ rotate: counterRotation }} className="flex flex-col items-center gap-1.5">
                  <PlayerAvatar displayName={p.displayName} avatarUrl={p.avatarUrl} size={68} />
                  <span className="max-w-[110px] truncate rounded-full bg-canvas-elevated/80 px-2.5 py-1 text-xs text-ink">
                    {p.displayName}
                  </span>
                </motion.div>
              </motion.div>
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center text-center">
        <p className="text-sm text-ink-muted">الجولة {state.round + 1}</p>
        {musicPlaying ? (
          <p className="mt-1 text-lg font-semibold">🎵 الموسيقى شغالة...</p>
        ) : (
          <>
            <p className="mt-1 text-lg font-semibold text-accent">اكتب رقم الكرسي دلوقتي!</p>
            {secondsLeft !== null && <p className="mt-0.5 text-2xl font-bold">{secondsLeft}</p>}
          </>
        )}
      </div>
    </div>
...
export function MusicalChairsGameView({
  state,
  chat,
  onNewRound,
}: {
  ...
  const snapshot = useRoundSnapshot(state);
        ...
        <AnimatePresence mode="wait">
          {state.phase === "WAITING_FOR_PLAYERS" && (
            ...
            >
              ...
              <h3 className="text-3xl font-bold">الكراسي الموسيقية</h3>
          ...
          {inRound && (
            ...
            >
              ...
              <div className="flex min-h-0 flex-col gap-4">
                <LeaderboardPanel round={snapshot.round} players={snapshot.players} />
                <ChatBox items={chat} />
              </div>
            </motion.div>
          )}
