    ...
    <div className="flex min-h-0 flex-[1.2] flex-col gap-1.5 overflow-y-auto rounded-2xl border border-glass-border bg-canvas-elevated/50 p-4">
      ...
      {ranked.map((p, i) => (
        <div key={p.handle} className={`flex items-center gap-2 rounded-xl px-2 py-1.5 ${i < 3 ? "bg-white/5" : ""}`}>
          ...
          <PlayerAvatar displayName={p.displayName} avatarUrl={p.avatarUrl} size={30} ring={false} />
          <bdi className="truncate text-sm">{p.displayName}</bdi>
          <span className="mr-auto shrink-0 text-sm font-bold text-cyan-400">{p.score}</span>
        </div>
      ))}
...
function FlagMedallion({ code, size }: { code: string; size: number }) {
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      />
      <div
        className="relative overflow-hidden rounded-full ring-4 ring-white/50 shadow-[0_0_30px_rgba(34,211,238,0.45)]"
...
function CapitalArena({ state }: { state: CapitalsState }) {
  ...
  return (
    <div className="relative flex h-[min(72vh,640px)] w-[min(72vh,640px)] flex-col items-center justify-center gap-6 overflow-hidden rounded-3xl border border-glass-border bg-gradient-to-br from-cyan-500/10 via-canvas-elevated to-canvas-elevated p-10">
      <motion.div
      ...
      <AnimatePresence mode="wait">
        {!revealed ? (
          ...
          >
            {state.countryCode && <FlagMedallion code={state.countryCode} size={168} />}
            <div className="flex flex-col items-center gap-1.5 text-center">
              <p className="flex items-center gap-1.5 text-sm font-semibold text-cyan-300">
                <Compass size={16} weight="fill" />
                عاصمة إيه؟
              </p>
              <p className="text-3xl font-extrabold text-white">{state.countryName}</p>
            </div>
          </motion.div>
        ) : (
          <motion.div
          ...
          >
            {state.countryCode && <FlagMedallion code={state.countryCode} size={100} />}
            <p className="text-sm text-white/60">{state.countryName}</p>
            <motion.div
            ...
            >
              <MapPinLine size={26} weight="fill" className="text-cyan-400" />
              <p className="text-3xl font-extrabold text-cyan-300">{state.capitalName}</p>
            </motion.div>
            {state.lastWinner ? (
              <motion.div
...
export function CapitalsGameView({
  state,
  chat,
  onNewRound,
}: {
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center">
        <AnimatePresence mode="wait">
          {state.phase === "WAITING_TO_START" && (
            ...
            >
              <Compass size={64} weight="fill" className="mx-auto mb-2 text-cyan-400" />
              <h3 className="text-3xl font-bold">عواصم</h3>
              <p className="mt-3 text-base text-ink-muted">
                {state.settings.totalRounds} دولة هيظهروا واحدة ورا التانية — هتبدأ أول دولة دلوقتي، جاوب باسم عاصمتها
                في الشات!
              </p>
            </motion.div>
          )}
