...
export function VsBadge() {
  return (
    <div className="relative z-10 flex h-40 w-40 shrink-0 items-center justify-center">
      <motion.div
        animate={{ opacity: [0.45, 0.85, 0.45], scale: [1, 1.15, 1] }}
        transition={{ duration: 1.8, repeat: Infinity, ease: "easeInOut" }}
        className="absolute h-28 w-28 rounded-full bg-gradient-to-br from-primary/70 to-fuchsia-500/70 blur-2xl"
      />
      {[0, 0.8].map((delay) => (
        <motion.div
          key={delay}
          ...
          className="absolute h-24 w-24 rounded-full border-2 border-fuchsia-400/70"
        />
      ))}
      <motion.svg
        ...
        className="absolute h-full w-full drop-shadow-[0_0_18px_rgba(217,70,239,0.7)]"
      >
        <defs>
          <linearGradient id="vs-star" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#c084fc" />
            <stop offset="55%" stopColor="#7c3aed" />
            <stop offset="100%" stopColor="#d946ef" />
          </linearGradient>
        </defs>
        <polygon points={VS_STAR_POINTS} fill="url(#vs-star)" stroke="rgba(255,255,255,0.35)" strokeWidth="1" />
      </motion.svg>
      <motion.div
        initial={{ scale: 0, rotate: -30 }}
        animate={{ scale: 1, rotate: 0 }}
        transition={{ type: "spring", stiffness: 280, damping: 11 }}
        className="relative flex h-[4.6rem] w-[4.6rem] items-center justify-center rounded-full border-2 border-white/40 bg-canvas-soft/90 shadow-[inset_0_2px_12px_rgba(192,132,252,0.35)]"
      >
        <span className="-skew-x-6 bg-gradient-to-b from-white via-[#e9d5ff] to-accent bg-clip-text text-4xl font-black italic text-transparent drop-shadow-[0_2px_8px_rgba(124,58,237,0.8)]">
          VS
        </span>
      </motion.div>
      <motion.span
        ...
        className="absolute -top-2 text-2xl drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]"
      >
        ⚡
      </motion.span>
      <motion.span
        ...
        transition={{ duration: 0.8, repeat: Infinity }}
        className="absolute -bottom-2 text-2xl drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]"
      >
        ⚡
      </motion.span>
    </div>
...
function VoterPile({ voters, max = 7 }: { voters: WouldYouRatherVoter[]; max?: number }) {
    ...
    <div className="flex h-8 items-center justify-center">
      <AnimatePresence>
        {shown.map((v, i) => (
          ...
          >
            <PlayerAvatar displayName={v.displayName} avatarUrl={v.avatarUrl} size={28} ring={false} />
          </motion.div>
        ))}
      </AnimatePresence>
    </div>
...
export function OptionCard({
  ...
  return (
    <motion.div
      layout
      animate={{
        scale: results && won ? 1.06 : dimmed ? 0.94 : 1,
    ...
    >
      <div
        className={`relative w-full overflow-hidden rounded-3xl border ${
          results && won
            ? "border-accent shadow-[0_0_50px_rgba(192,132,252,0.55)]"
            : "border-white/15 shadow-[0_10px_40px_rgba(5,3,13,0.5)]"
        }`}
      ...
      >
        ...
        <div className="relative flex flex-col items-center justify-center gap-3 p-10">
          <div className="relative flex h-32 w-32 items-center justify-center">
            ...
            <motion.span
              ...
              className="relative text-7xl drop-shadow-[0_8px_18px_rgba(5,3,13,0.55)]"
            >
              {option.emoji}
            </motion.span>
          </div>
          <p className="text-center text-2xl font-extrabold leading-snug text-white drop-shadow-[0_2px_6px_rgba(5,3,13,0.7)]">
            {option.label}
          </p>

          <div className="w-full max-w-xs">
            <div className="h-3 w-full overflow-hidden rounded-full bg-[#05030d]/45 ring-1 ring-white/15">
              <motion.div
                animate={{ width: `${pct}%` }}
            ...
            <div className="mt-1.5 flex items-center justify-between text-sm font-bold text-white/90">
              <span>{votes} صوت</span>
              <span>{pct}%</span>
            </div>
          </div>
        ...
        {results && won && (
          <motion.div
            ...
            className="absolute right-3 top-3 rounded-full bg-accent px-3 py-1 text-sm font-extrabold text-canvas shadow-glow-sm"
          >
            🏆 الأغلبية هنا!
          </motion.div>
        )}
      ...
      <div className="flex flex-col items-center gap-0.5">
        <div className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-primary to-fuchsia-600 text-3xl font-black text-white ring-2 ring-white/30 shadow-glow-sm">
          {number}
        </div>
        <p className="text-xs text-ink-muted">اكتب {number} في الشات</p>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <div className="flex h-full w-full max-w-5xl flex-col items-center justify-center gap-8">
      <div className="flex flex-col items-center gap-2 text-center">
        <p className="rounded-full bg-white/10 px-3 py-1 text-sm text-ink-muted">سؤال {state.round}</p>
        <motion.h2
          key={state.round}
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      <p className="text-sm text-ink-muted">إجمالي الأصوات: {total} 🔥</p>
    </div>
  );
}

export function WouldYouRatherGameView({
  state,
  chat,
  onNewRound,
}: {
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center">
        <AnimatePresence mode="wait">
          {state.phase === "WAITING_TO_START" && (
            ...
            >
              <Scales size={64} weight="fill" className="mx-auto mb-2 text-accent" />
              <h3 className="text-3xl font-bold">إما / أو</h3>
              <p className="mt-3 text-base text-ink-muted">هيبدأ أول سؤال دلوقتي — صوّت بكتابة 1 أو 2 في الشات!</p>
            </motion.div>
          )}
          ...
          {state.phase === "FINISHED" && (
            ...
            >
              {/* winner is always null in this game — the payoff is the votes, not a person. */}
              <NoWinnerScreen message="خلصت الأسئلة — شكراً لكل اللي صوّتوا! 🎭" onNewRound={onNewRound} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
